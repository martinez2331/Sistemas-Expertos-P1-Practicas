...
def create_population(pop_size, gene_length):
    ...
    for _ in range(pop_size):
        individual = ''.join(random.choice("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") for _ in range(gene_length))
...
def select_parents(population, num_parents, target):
    ...
    for _ in range(num_parents):
        parents.append(max(population, key=lambda x: fitness(x, target)))
...
def mutate(individual, mutation_rate):
    ...
    for gene in individual:
        if random.random() < mutation_rate:
            mutated_individual += random.choice("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
        else:
...
for generation in range(num_generations):
    parents = select_parents(population, num_parents, target_password)
    ...
    best_individual = max(population, key=lambda x: fitness(x, target_password))
    print(f"Generación {generation}: {best_individual} (Fitness: {fitness(best_individual, target_password)})")

# Resultado final
best_individual = max(population, key=lambda x: fitness(x, target_password))
print("Mejor individuo encontrado:", best_individual)
print("Fitness:", fitness(best_individual, target_password))
